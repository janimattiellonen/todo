...
- [ ] **Test database isolation strategy.** Per-test transaction rollback vs per-test truncate-and-reseed vs schema-per-test. Decide when writing the first integration test — let the choice be informed by what's actually painful. The reference project uses Vitest `isolate: true, fileParallelism: false` for the Integration tests project — copy that posture; the in-test strategy is the open part.
- [ ] **Hosting vendor (and EU region).** Re-opens when "local-only" no longer suffices. Plan §13 already lists this.
- [ ] **Email-template authoring** — React-Email components vs hand-rolled HTML strings. Decide when writing the first email.
...
- [ ] **StyleX implementation strategy.** Token file layout (single `app/ui/tokens.stylex.ts` vs split files), theming scaffold via `stylex.defineVars`, per-component `stylex.create` placement (bottom-of-file per `project-conventions`). **Resolved at the Pre-Phase-2 design review** (immediately after Phase 1, consuming Phase 0's tokens).
