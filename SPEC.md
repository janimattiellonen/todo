...
| 2026-05-23 | **`@dnd-kit/core` + `@dnd-kit/sortable`** for drag-and-drop, pointer sensor with `activationConstraint: { distance: 8px }` | Best-maintained DnD library for React; sensor architecture supports pointer now and keyboard later (#41) cleanly; the 8px activation distance prevents accidental drags on click. Server-confirmed move with optimistic local reorder so the card doesn't snap back during the round-trip. | Native HTML5 DnD (rejected — janky API, weak mobile, hard to style the drag image); react-aria drop-zones (rejected — heavier API, less commonly used for kanban, not the plan's recommendation) |
...
- [x] ~~**Drag-and-drop library.**~~ *Resolved 2026-05-23: **`@dnd-kit/core` + `@dnd-kit/sortable`**. Pointer sensor with `activationConstraint: { distance: 8px }` so clicks don't accidentally trigger drags. Keyboard sensor deferred to the a11y audit (#41). See Decision Log row 2026-05-23.*
