...
function uniqueEmail(): string {
  ...
  return `board-delete-${stamp}@example.com`;
}
...
test("delete requires confirmation; Cancel reverts; Confirm removes the task", async ({
  page,
}) => {
  ...
  try {
    ...
    await todo.getByPlaceholder("Title").fill("Doomed");
    ...
    // Step 1: click Delete → confirmation prompt appears, task NOT yet removed.
    await dialog.getByTestId("delete-task-toggle").click();
    await expect(dialog.getByText(/delete this task/i)).toBeVisible();
    await expect(dialog.getByTestId("delete-task-confirm")).toBeVisible();
    await expect(todo.getByTestId("board-task")).toHaveCount(1);

    // Step 2: Cancel → footer reverts, task still present.
    await dialog.getByTestId("delete-task-cancel").click();
    await expect(dialog.getByTestId("delete-task-toggle")).toBeVisible();
    await expect(todo.getByTestId("board-task")).toHaveCount(1);

    // Step 3: Delete → Confirm → task removed, modal closes.
    await dialog.getByTestId("delete-task-toggle").click();
    await dialog.getByTestId("delete-task-confirm").click();
